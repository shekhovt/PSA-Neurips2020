...
class odict(dict):
    ...
    def __setattr__(self, name, value):
        if self.__dict__['locked'] and name not in self:
            raise ValueError('No attribute {} and adding new keys is locked'.format(name))
        self[name] = value
    ...
    def lock(self):
        """
        after locking, silent adding of new keys is not allowed
        """
    ...
    def __sub__(self, other):
        """ dict difference """
        if len(self) == 0 and len(other == 0):  # both empty dicts ok
            return odict()
        r = odict()
        for (k, v) in self.items():
            if isinstance(v, odict):
    ...
    @staticmethod
    def intersection(self, *dicts):
        if len(dicts) == 0:
            return copy.deepcopy(self)
        assert (None not in dicts)
        r = odict()
        for (k, v) in self.items():
            if isinstance(v, odict):
                sd = [*(d[k] for d in dicts if k in d)]
                if all(isinstance(v1, odict) for v1 in sd):
                    if len(sd) == 0 and len(v) == 0:  # both empty, ok, include empty
                        r[k] = v
                    elif len(sd) > 0:
                        r1 = odict.intersection(v, *sd)
                        if len(r1) > 0:
                            r[k] = r1
            else:
                if all(k in d.keys() and v == d[k] for d in dicts):
                    r[k] = v
        return r
