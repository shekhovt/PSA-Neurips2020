...
from .architectures import GumbelConv2d
...
def _resnet(arch, block, layers, pretrained, progress, device, **kwargs):
    ...
    if pretrained:
        state_dict = torch.load(f'{root}/models/state_dicts/'+arch+'.pt', map_location=device)
...
def gumbel_conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    """3x3 convolution with padding"""
    return GumbelConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation)


def gumbel_conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return GumbelConv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)


class GumbelBasicBlock(nn.Module):
    expansion = 1
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = gumbel_conv3x3(inplanes, planes, stride)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = gumbel_conv3x3(planes, planes)
        self.downsample = downsample
        self.stride = stride
